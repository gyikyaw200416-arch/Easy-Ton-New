...
function App() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    // Updated Spin Cooldown to 1 Hour (3,600,000ms)
  ...
  const handleGlobalClick = () => {
    if (isAdWatching && adTimer > 0) {
      alert(`Finish the advertisement first! ${adTimer}s remaining.`);
  ...
  const handleSpin = async () => {
    if (user.id !== ADMIN_ID && timeLeft > 0) return alert(`Cooldown active! Wait ${Math.ceil(timeLeft/60000)} mins.`);
    ...
    <div style={styles.container} onClick={handleGlobalClick}>
      ...
      {isAdWatching && (
        <div style={styles.adOverlay}>
          <h2 style={{color: '#facc15'}}>ADVERTISEMENT IN PROGRESS</h2>
          <div style={{fontSize: 50, fontWeight: 'bold', margin: '20px 0'}}>{adTimer}s</div>
          <p>Stay on the advertisement page.<br/>Reward will be added after timer finishes.</p>
        </div>
      )}
                ...
                <div style={{background:'#f0f9ff', padding:15, borderRadius:10, border:'1px solid #000', marginBottom:10}}>
                  ...
                  {userWithdraws.map(w => (
                    <div key={w.id} style={{fontSize:11, marginBottom:10}}>
                        ...
                        <button onClick={() => approveWithdraw(w.id)} style={{float:'right', background:'blue', color:'#fff', padding: '2px 5px', borderRadius: 5}}>Success</button>
                    </div>
                  ))}
                </div>
              )}
